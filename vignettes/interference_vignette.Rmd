...
### ZSRE and YSRE
If the data is not already pre-processed, the `ZSRE` and `YSRE` functions can be run to output the necessary format for the binary treatment assignment variable (`Z`) and the vector of outcomes (`Y`) for any desired variable of interest respectively. The inputs for `ZSRE` are the binary assignment vector and the vector of cluster labels. The inputs for `YSRE` are the vector of cluster labels and the outcomes of the variable of interest.

In this case, we are interested in long-term fixed contracts and permanent contracts. We acquire the necessary binary treatment assignment variable as follows:

```{r}
Z <- ZSRE(data$assigned, data$anonale)
```

We can acquire the necessary list of outcomes `Y` as follows:

```{r}
Y.LTFC <- YSRE(data$anonale, data$cdd6m)
Y.perm <- YSRE(data$anonale, data$cdi)
```
...
Then, we specify the effect size and use the `Calsamplesize` function to calculate the appropriate sample sizes for the permanent contract and the LTFC. The default `alpha`(significance level) and `beta` (power) are set at 0.05 and 0.2 respectively. The output of `n.avg` may need to be rounded, in order to have it be integer valued.

```{r}
# round n.avg
n.avg <- round(var.perm$n.avg)

# calculate sample size for the permanent contract
print("Permanent Contract:")
round(Calsamplesize(0.03,n.avg,qa, pa, r=var.perm$r, sigma.perm, alpha=0.05, beta=0.2))
save <- Calsamplesize(0.03,n.avg,qa, pa, r=var.perm$r, sigma.perm, alpha=0.05, beta=0.2)
print(save)

# calculate sample size for the long term fixed contract
print("Long Term Fixed Contract")
round(Calsamplesize(0.03,n.avg,qa, pa, r=var.LTFC$r, sigma.LTFC, alpha=0.05, beta=0.2))
```
