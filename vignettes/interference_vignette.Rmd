...
4. `Calsamplesize`: returns a list of the calculated necessary sample sizes in order to achieve a given power level at a given significance level for the three types of effects.
...
### CalAPO
In order to calculate a list of point estimates and variances for an effect of interest, run the CalAPO command. It is necessary first to extract the necessary vectors from the data. The code below extracts `Z`, the vector of treatment assignments, `A`, the vector of treatment assignment mechanisms and `Y.perm`, the vector of potential outcomes for the variable of interest. In this case, we are interested in long-term fixed contracts, and permanent contracts.
