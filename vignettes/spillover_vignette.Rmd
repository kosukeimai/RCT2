...
# Data
The data set is a subset of data from the randomized evaluation of the India's National Health Insurance Program (RSBY). The data initially contained six variables as listed below:

`village_id`: the village id

`DistrictId`: the district id

`treat`: the treatment status

`mech`: the treatment assignment mechanism

`enrolled`: enrollment in RSBY

`EXPhosp_1`: hospital expenditure.

After processing the for the purposes of the package, there remain four variables of intrest which we remained for the purposes of analysis:
